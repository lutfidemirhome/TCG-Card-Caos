#!/usr/bin/env python3
"""Generate CardDefinition assets for Normal Common card PNGs."""
import os
import re
import uuid

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
ART_ROOT = os.path.join(ROOT, "Assets/Art/Cards/Normal_Common_Cards")
...
def main():
    pattern = re.compile(r"normal_common_(?P<character>[a-z]+)_(?P<slot>\d+)\.png$", re.IGNORECASE)
    created = 0

    for dirpath, _, filenames in os.walk(ART_ROOT):
        for filename in sorted(filenames):
            if not filename.endswith(".png") or filename.endswith(".png.meta"):
                continue
            ...
            definition_id = f"normal_common_{character}_{slot:02d}"
            png_path = os.path.join(dirpath, filename)
            texture_guid = read_guid(png_path + ".meta")
    ...
    print(f"Generated {created} CardDefinition assets in {OUT_DIR}")
