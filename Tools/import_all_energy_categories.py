#!/usr/bin/env python3
"""Import all_energy cards and one 100-slot (common-size) cabinet.

Desktop PNGs are named energy_{type}_{slot}.png (not all_energy_*).
...
PNG_RE = re.compile(
    r"^energy_(?P<character>[a-z0-9_]+)_(?P<slot>\d+)\.png$",
    re.IGNORECASE,
)
...
CATEGORIES = [
    {
        "id": "all_energy",
        "desktop": "all_energy",
        "art_folder": "All_Energy_Cards",
        "cab_dir": "Cabinets_Energy",
        "prefab": "Cabinets_AllEnergy",
        "instance_id": 910000701,
        "pos": (12.1, 0.06000042, -16.84),
    },
]
...
def copy_sign_and_category(dst_id: str):
    from shelf_sign_paths import TEXTURES_ROOT, MATERIALS_ROOT, material_path, texture_path

    src_tex_stem = "fire_common_sign"
    dst_tex_stem = f"{dst_id}_sign"
    src_tex = texture_path(src_tex_stem)
    dst_tex = TEXTURES_ROOT / "Energy" / f"{dst_tex_stem}.png"
    ...
    dst_mat = MATERIALS_ROOT / "Energy" / f"{dst_tex_stem}.mat"
...
def place_cabinets_in_scene():
    ...
    if "Cabinets_AllEnergy" in scene_text:
        print("MainScene already has all energy cabinet; rewiring guids if needed")
    ...
    SCENE_PATH.write_text(scene_text, encoding="utf-8")
    print("Placed 1 all energy cabinet in MainScene")
...
def main():
    ...
    print("== Copy 100-slot cabinet ==")
    copy_cabinets()
    print("== Place cabinet in MainScene ==")
    place_cabinets_in_scene()
    print("Done. All energy is catalog + cabinet only (no ground spawn).")
