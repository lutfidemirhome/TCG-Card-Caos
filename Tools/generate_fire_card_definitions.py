#!/usr/bin/env python3
"""Generate CardDefinition assets for Fire Common / Uncommon / Rare card PNGs."""
import os
import re
import uuid
...
SETS = [
    {
        "art_root": "Assets/Art/Cards/Fire_Common_Cards",
        "category": "fire_common",
        "max_slot": 10,
        "slot_width": 2,
        "patterns": [
            re.compile(r"^fire_(?P<character>[a-z]+)_(?P<slot>\d+)\.png$", re.IGNORECASE),
            re.compile(r"^Ateş_(?P<character>[a-z]+)_(?P<slot>\d+)\.png$", re.IGNORECASE),
            re.compile(r"^Ateş_(?P<character>[a-z]+)_(?P<slot>\d+)\.png$", re.IGNORECASE),
        ],
    },
    {
        "art_root": "Assets/Art/Cards/Fire_Uncommon_Cards",
        "category": "fire_uncommon",
        "max_slot": 5,
        "slot_width": 0,
        "patterns": [
            re.compile(r"^fire_(?P<character>[a-z]+)_(?P<slot>\d+)\.png$", re.IGNORECASE),
    ...
    {
        "art_root": "Assets/Art/Cards/Fire_Rare_Cards",
        "category": "fire_rare",
        "max_slot": 3,
        "slot_width": 0,
        "patterns": [
...
def main():
    ...
    print(f"Generated {total} Fire CardDefinition assets in {OUT_DIR}")
