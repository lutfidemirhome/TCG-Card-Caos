...
namespace Grabbit
{
    public class GrabbitEditor : EditorWindow
    {
        ...
        private void RegisterSceneMeshes()
        {
            ...
            var settings = GetOrFetchSettings();
            if (!settings)
                return;

            //this runs on every scene open, so only complain about a missing container when the concave colliders
            //it feeds are actually switched on
            var container = GetOrFetchColliderMeshContainer(!settings.UseDynamicNonConvexColliders);
            if (!container)
                return;

            container.RegisterCollidersFromMeshFiltersInScene(settings, filters);
        }

        //single resolution point for the container, because losing it costs the whole concave collider feature:
        //every grab drops to one convex hull, and dense meshes then trip PhysX's 256 polygon hull limit. It used
        //to be resolved only by FetchSettings(), which PrepareSettings() skips as soon as anything else has
        //populated the static CurrentSettings - and the scene view toolbar icon and the tool activation path both
        //do that first - so the window's serialized field routinely stayed null for the whole session. On a fresh
        //install there was nothing to find in the first place, see the note on creation below.
        private static ColliderMeshContainer cachedColliderMeshContainer;
        private static bool warnedMissingColliderMeshContainer;

        public static ColliderMeshContainer GetOrFetchColliderMeshContainer(bool noWarning = false)
        {
            if (cachedColliderMeshContainer)
                return cachedColliderMeshContainer;

            if (IsInstanceCreated && instance.ColliderMeshContainer)
            {
                cachedColliderMeshContainer = instance.ColliderMeshContainer;
                return cachedColliderMeshContainer;
            }

            foreach (var id in AssetDatabase.FindAssets("t:ColliderMeshContainer"))
            {
                //an installed Grabbit 2 answers to the same type name with its own container, which never loads
                //as this type - so keep looking instead of trusting the first hit
                var container = AssetDatabase.LoadAssetAtPath<ColliderMeshContainer>(AssetDatabase.GUIDToAssetPath(id));
                if (!container)
                    continue;

                cachedColliderMeshContainer = container;
                break;
            }

            //the .unitypackage deliberately ships no container: it is mutable per-project state, and a same-GUID
            //import would wipe the customer's baked colliders on every update (see JunglePublisher's
            //MutableStateAssetNames, which strips it by file name). That trade only works when the plugin
            //recreates it on first run - Grabbit 2 does, this one never did, so a fresh install had no container
            //at all and every grab silently fell back to a single convex hull. Only created for a caller that
            //actually needs it, never from the passive lookups.
            if (!cachedColliderMeshContainer && !noWarning)
                cachedColliderMeshContainer = CreateColliderMeshContainer();

            if (cachedColliderMeshContainer)
            {
                //keep the window's serialized reference in sync so the inspector and the next session agree
                if (IsInstanceCreated)
                    instance.ColliderMeshContainer = cachedColliderMeshContainer;

                warnedMissingColliderMeshContainer = false;
                return cachedColliderMeshContainer;
            }

            //losing the container is never a user choice: every object silently drops to one convex hull, so say it
            if (!noWarning && !warnedMissingColliderMeshContainer)
            {
                warnedMissingColliderMeshContainer = true;
                Debug.LogError(
                    "Grabbit: the collider container asset is missing and could not be created right now, so " +
                    "concave colliders are unavailable and objects are moved with a single rough convex hull " +
                    "instead of their real shape. Grab again once Unity has finished importing and compiling, or " +
                    "create one through Assets > Create > Tools > Grabbit > Create New Collider Container.");
            }

            return null;
        }

        /// <summary>Creates the baked collider cache the package deliberately does not ship, at the Grabbit
        /// plugin root. Returns null while the asset database is busy, so the caller falls back to its normal
        /// missing-container handling and no duplicate appears next to one that is still being imported.</summary>
        private static ColliderMeshContainer CreateColliderMeshContainer()
        {
            if (EditorApplication.isUpdating || EditorApplication.isCompiling)
                return null;

            var asset = CreateInstance<ColliderMeshContainer>();
            var path = AssetDatabase.GenerateUniqueAssetPath(PluginRootFolder() + "/Baked Colliders.asset");
            AssetDatabase.CreateAsset(asset, path);
            AssetDatabase.SaveAssets();

            Debug.Log($"Grabbit: created the baked collider cache at {path}.", asset);
            return asset;
        }

        /// <summary>The Grabbit plugin root, anchored on this script's own asset path
        /// (&lt;root&gt;/Editor/Scripts/GrabbitEditor.cs) so it holds wherever the customer moved the folder.</summary>
        private static string PluginRootFolder()
        {
            foreach (var guid in AssetDatabase.FindAssets("GrabbitEditor t:MonoScript"))
            {
                var scriptPath = AssetDatabase.GUIDToAssetPath(guid);
                var script = AssetDatabase.LoadAssetAtPath<MonoScript>(scriptPath);

                //Grabbit 2 ships a GrabbitEditor of its own, so match on the resolved type rather than the name
                if (!script || script.GetClass() != typeof(GrabbitEditor))
                    continue;

                var root = Path.GetDirectoryName(Path.GetDirectoryName(Path.GetDirectoryName(scriptPath)));
        ...
        private void FetchSettings()
        {
            ...
            //the container lookup must not sit behind the style init: styles are a no-op outside of OnGUI, and
            //losing the container here would silently disable generated colliders for the rest of the session.
            //this path is no longer the only resolution point - see GetOrFetchColliderMeshContainer - but the
            //window still primes it so its serialized reference is filled as soon as Grabbit is opened
            GetOrFetchColliderMeshContainer(true);
        ...
        {
            var container = GetOrFetchColliderMeshContainer();
            if (!container)
                return;

            container.GenerateAllColliders(CurrentSettings);
        }
        ...
        private void ConfirmDeletion()
        {
            if (EditorUtility.DisplayDialog("Clearing the colliders",
                    ...
                    "Ok", "Cancel"))
            {
                //quiet lookup: creating a cache just to empty it, or complaining that there is none to empty,
                //would both be noise
                var container = GetOrFetchColliderMeshContainer(true);
                if (container)
                    container.ClearColliders();
            }
        }
