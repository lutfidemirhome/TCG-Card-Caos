    ...
    public class GrabbitTool : EditorTool
    {
        ...
        private void DisableLogic(bool butDontChangeTools = false)
        {
            ...
            try
            {
                ...
                //every caller of this is a teardown path that fires while the editor is pulling the rug out -
                //compilation starting, quitting, a scene being saved - and the settings asset can already be
                //unloaded by then. Its managed fields still read back fine on a destroyed ScriptableObject, so
                //the old code sailed straight past them and only broke at SetDirty, which goes native: an
                //ArgumentNullException surfaced to the user as "contact us on Discord" for a teardown that
                //needed no support at all. Unity's implicit bool sees the destroyed object; == null does not.
                if (!settings)
                    settings = GrabbitEditor.GetOrFetchSettings(true);
        ...
        private void EnableLogic()
        {
            try
            {
                ...
                colliderMeshContainer = GrabbitEditor.GetOrFetchColliderMeshContainer(true);
        ...
        {
            DisableLogic();
            //same teardown-time hazard as in DisableLogic: the asset may be gone by the time we get here
            if (settings && settings.DisplayWarning && Selection.gameObjects.Length > 0)
            {
/*                Debug.LogWarning(
        ...
        {
            //this is also the catch-handler's last resort, so it must not be the thing that throws. With no
            //settings left to read the preference from, put back exactly what was captured on activation: that
            //is the project's own configuration, and leaving the editor on Script simulation mode with Grabbit's
            //solver counts is the one outcome worth avoiding here.
            if (!settings || !settings.ResetToDefaultPhysicsValues)
            {
#if UNITY_2022_3_OR_NEWER
        ...
        public override void OnToolGUI(EditorWindow window)
        {
            try
            {
                ...
                //the scene view can draw before the Grabbit window repaints, and the hotkey path cannot build
                //the styles itself (it runs outside of OnGUI), so make sure they exist before using them here
                settings.InitializeStyle();
