...
namespace Grabbit
{
    ...
    public class GrabbitSettings : ScriptableObject
    {
        ...
        public void InitializeStyle(bool force = false)
        {
            ...
            //GUI.skin and EditorStyles can only be touched from inside an IMGUI context. The hotkeys go through
            //MenuItem callbacks, which Unity dispatches outside of OnGUI: bail out here and let the next OnGUI
            //build the styles, rather than throwing and aborting whatever the caller was doing.
        ...
        //the shipped settings asset and the default preset both carry 10 here, so these initializers only ever
        //apply to a settings asset someone creates from scratch through the Create menu - which used to come out
        //at 1/1, below Unity's own 6/1 defaults, and resolved contacts noticeably worse than the shipped asset
        //for no stated reason. Matched to what we actually ship instead.
