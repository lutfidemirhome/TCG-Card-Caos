				...
				if (materialEditor.isVisible && !mShader.hasMultipleDifferentValues && mShader.objectReferenceValue != null)
				{
					...
					foreach(var p in properties)
					{
						...
						else
						{
							...
							if (visible && (TCP2_MaterialPropertyCompat.GetPropertyFlags(p) & (ShaderPropertyFlags.PerRendererData | ShaderPropertyFlags.HideInInspector)) == ShaderPropertyFlags.None)
							{
